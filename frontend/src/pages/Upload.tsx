...
function mappingToRoleMap(m: ColumnMapping): Record<number, ColumnRole> {
  ...
  if (m.date_description_col != null)
    out[m.date_description_col] = "date_description";
      ...
      <div className="max-w-4xl space-y-5">
        <div className="bg-slate-900 rounded-xl border border-green-800 p-4 flex items-center justify-between">
          <p className="text-green-400 text-sm font-medium">
            {result.total_added} transaction
            {result.total_added !== 1 ? "s" : ""} imported
            {result.total_skipped > 0 && (
              <span className="text-slate-500 font-normal">
                {" "}
                · {result.total_skipped} duplicates skipped
              </span>
            )}
            {result.total_errors > 0 && (
              <span className="text-red-400 font-normal">
                {" "}
                · {result.total_errors} file
                {result.total_errors !== 1 ? "s" : ""} failed
              </span>
            )}
          </p>
          <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800 text-xs text-slate-500 uppercase tracking-wide">
                ...
                <th className="px-4 py-2.5 text-right font-medium w-20">
                  Added
                </th>
                <th className="px-4 py-2.5 text-right font-medium w-20">
                  Skipped
                </th>
                <th className="px-4 py-2.5 text-right font-medium w-16">
                  Status
                </th>
              </tr>
            </thead>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800 p-4 space-y-3">
        ...
        {needsYear && (
          <div>
            ...
            />
            <p className="text-xs text-slate-500 mt-1">
              All selected files will use this year
            </p>
          </div>
        )}
      </div>
              ...
              <tbody>
                {result.transactions.map((t) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-2 text-slate-300 truncate max-w-[280px]">
                      {t.description}
                    </td>
                    <td
                      className={`px-4 py-2 text-right tabular-nums font-medium ${t.amount >= 0 ? "text-green-400" : "text-red-400"}`}
                    >
              ...
              <tbody>
                {sample_rows.map((row, rowIdx) => (
                  <tr
                    key={rowIdx}
                    className={
                      rowIdx % 2 === 0 ? "bg-slate-900" : "bg-slate-800/40"
                    }
                  >
